...
object PL2 extends App {
  ...
  def gravedadIzquierda(tablero: List[List[Int]]): List[List[Int]] = {
    def gravedadIzquierdaAux(tablero: List[List[Int]]): List[List[Int]] = {
      def crearListaDeListaCeros(i: Int, j: Int): List[List[Int]] = i match {
        case 0 => List[List[Int]]()
        case _ => crearListaCeros(j) :: crearListaDeListaCeros(i - 1, j)
      }
      ...
      if (tablero.isEmpty)
        ...
        gravedadIzquierdaAux(tablero.tail)
      else
        tablero.head :: gravedadIzquierdaAux(tablero.tail)
    }
    gravedadIzquierdaAux(tablero)
  }
  ...
  def eliminarElementos(tablero: List[List[Int]], casillas: List[List[Int]]): List[List[Int]] ={
    ...
    else actualizar(eliminarElementos(eliminarElemento(tablero, casillas.head.head, casillas.head.tail.head), casillas.tail))
  }
