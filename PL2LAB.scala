...
object PL2LAB extends App {
  ...
  def contiene[T](tablero : List[T], elemento : T) : Boolean = {
    if(tablero.isEmpty) false
      ...
      if(tablero.head == elemento) true
      else contiene(tablero.tail, elemento)
    }
  }
  ...
  def gravedad(tablero: List[List[Int]]): List[List[Int]] = {
    ...
    tablero match {
      ...
      case head :: tail => gravedadColumna(head) :: gravedad(tail)
    }

  }
  def sustituirCeros(tablero: List[List[Int]],colores:Int): List[List[Int]] = {
    def sustituirCerosColumna(columna: List[Int]): List[Int] = columna match{

      case Nil => List[Int]()
      case 0 :: tail => scala.util.Random.nextInt(colores)+1 :: sustituirCerosColumna(tail)
      case head :: tail => head :: tail
    }

    tablero match {
      case Nil => List[List[Int]]()
      case head :: tail => sustituirCerosColumna(head) :: sustituirCeros(tail,colores)
    }

  }
  // Actualizar tablero
  def actualizar(tablero: List[List[Int]],colores:Int): List[List[Int]] = {
    val nuevoTablero = gravedad(tablero)

    println("Tablero tras eliminar fichas")
    mostrarTablero(tablero)
    print("\n")
    println("Tablero sin insertar fichas nuevas")
    mostrarTablero(nuevoTablero)
    print("\n")
    sustituirCeros(nuevoTablero,colores)
  }
  ...
  def eliminarElementos(tablero: List[List[Int]], casillas: List[List[Int]],colores:Int): List[List[Int]] ={
    def eliminarElemento(tablero: List[List[Int]], x: Int, y: Int): List[List[Int]] = {
      def eliminarElementoAux(columna: List[Int], y: Int): List[Int]= {
        if (x >= 0 && y >= 0 && x < longitudLista(tablero) && y < longitudLista(tablero.head)) {

          if (y == 0) {
            0 :: columna.tail
          } else  {
            columna.head :: eliminarElementoAux(columna.tail, y - 1)
          }
        }else Nil
      }
      ...
      if (x >= 0 && y >= 0 && x < longitudLista(tablero) && y < longitudLista(tablero.head)) {
        ...
        if (x == 0) {
          eliminarElementoAux(tablero.head, y) :: tablero.tail
        } else {
          tablero.head :: eliminarElemento(tablero.tail, x - 1, y)
        }
      } else Nil
    }

    if(tablero.isEmpty) Nil
    else if (casillas.isEmpty) tablero
    else  eliminarElementos(eliminarElemento(tablero, casillas.head.head, casillas.head.tail.head), casillas.tail,colores)

  }

  // Borra del tablero una casilla dada y los elementos adyacentes
  def explosionBomba(tablero: List[List[Int]], x: Int, y: Int,colores:Int): List[List[Int]] = {
    ...
    actualizar(eliminarElementos(tablero,alcanceBomba(tablero,x,y),colores),colores)
  }
  ...
  def jugar(tablero: List[List[Int]] ,vidas:Int,puntos:Int,sizeX:Int,sizeY:Int,bombas:Int,colores:Int,partidas:Int=1): Int ={
    ...
    def jugarAux(tablero: List[List[Int]] , x: Int, y:Int,vidas:Int,puntos:Int,partidas:Int): Int={
      ...
      if(buscarElemento(tablero,x,y) == 8){
        jugar(explosionBomba(tablero,x,y,colores),vidas,puntos,sizeX,sizeY, bombas, colores, partidas)
      }else if (longitudLista(vecinos)>=3) jugar(eliminarElementos(tablero,vecinos,colores),vidas,puntos+longitudLista(vecinos)*10,sizeX,sizeY, bombas, colores, partidas)
      else jugar(eliminarElementos(tablero,List(List(x,y)),colores),vidas-1,0,sizeX,sizeY,bombas,colores,partidas)
    }
  ...
  def jugarAI(tablero: List[List[Int]] ,vidas:Int,puntos:Int,sizeX:Int,sizeY:Int,bombas:Int,colores:Int,partidas:Int=1): Int ={
    ...
    def jugarAIAux(tablero: List[List[Int]] , x: Int, y:Int,vidas:Int,puntos:Int,partidas:Int): Int={
      val vecinos= colindantes(tablero,x,y)
      if(buscarElemento(tablero,x,y) == 8){
        jugarAI(explosionBomba(tablero,x,y,colores),vidas,puntos,sizeX,sizeY, bombas, colores, partidas)
      }else if (longitudLista(vecinos)>=3) jugarAI(actualizar(eliminarElementos(tablero,vecinos,colores),colores),vidas,puntos+longitudLista(vecinos)*10,sizeX,sizeY, bombas, colores, partidas)
      else jugarAI(actualizar(eliminarElementos(tablero,List(List(x,y)),colores),colores),vidas-1,0,sizeX,sizeY,bombas,colores,partidas)
    }
    ...
    def decidirPosicionAux(tablero:List[List[Int]],maximo:Int,elemento: List[Int],coords:List[List[Int]], visitados:List[Any]= Nil): List[Int] ={
      if (coords.isEmpty) {
        if (maximo < 3) {
        ...
        if (buscarElemento(tablero,coords.head.head,coords.head.tail.head) == 0) decidirPosicionAux(tablero,maximo,elemento,coords.tail,visitados:::colindantes(tablero,coords.head.head,coords.head.tail.head))
        else {
          if (contiene(visitados,coords.head)) decidirPosicionAux(tablero,maximo,elemento,coords.tail,visitados)
          else if (colindantes(tablero,coords.head.head,coords.head.tail.head).length > maximo) {
            decidirPosicionAux(tablero,colindantes(tablero,coords.head.head,coords.head.tail.head).length,coords.head,coords.tail,visitados:::colindantes(tablero,coords.head.head,coords.head.tail.head))
          }
          else decidirPosicionAux(tablero,maximo,elemento,coords.tail,visitados:::colindantes(tablero,coords.head.head,coords.head.tail.head))
        }
      }
    ...
    if(vidas>0 ){
      if(!tablero.isEmpty) {
        ...
        print("\n")
        print(s"Vidas: $vidas Puntos: $puntos Partidas: $partidas\n")
        val jugada = decidirPosicionAux(tablero,0,List(0,0),obtenerCoordenadasNoVacías(tablero))
        println("Enter para siguiente paso\n")
        ...
        print("\n")
        print(s"Vidas: $vidas Puntos: $puntos Partidas: $partidas\n")
        val jugada = decidirPosicionAux(t,0,List(0,0),obtenerCoordenadasNoVacías(t))
        println("Enter para siguiente paso\n")
        readLine()
